...
import { Award, Clock, RotateCcw, Video, Activity, Zap } from 'lucide-react';
...
import { VBTDegradationChart } from './VBTDegradationChart';
...
export const SummaryScreen: React.FC<SummaryScreenProps> = ({ stats, leveling, onRestart, onViewReplay }) => {
  ...
  const averageRepScore = stats.repScores.length > 0 ? Math.round(stats.repScores.reduce((a, b) => a + b, 0) / stats.repScores.length) : 0;
  const streakData = updateWorkoutStreak();

  if (stats.totalReps === 0) {
    return (
      <div className="screen-container" style={{ background: "radial-gradient(ellipse at 50% 20%, #151b4d 0%, var(--bg-primary) 70%)", padding: "60px 40px", alignItems: "center", justifyContent: "center" }}>
        ...
        <div className="animate-in" style={{ display: "flex", gap: "20px", width: "100%", maxWidth: "300px", pointerEvents: "all" }}>
          <button onClick={onRestart} className="btn-neon" style={{ flex: 1, background: "var(--neon-purple)", color: "#fff" }}><RotateCcw size={16} /> RESTART SESSION</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="screen-container" style={{ background: "radial-gradient(ellipse at 50% 20%, #151b4d 0%, var(--bg-primary) 70%)", padding: "60px 40px", alignItems: "center", overflowY: "auto" }}>
      ...
      <div className="glass animate-in" style={{ width: "220px", height: "220px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", marginBottom: "30px", border: `1px solid ${accuracyColor}`, boxShadow: `0 0 30px ${accuracyColor}33` }}>
        <svg width="180" height="180" viewBox="0 0 160 160" style={{ transform: "rotate(-90deg)" }}>
          ...
          <circle cx="80" cy="80" r="70" fill="none" stroke={accuracyColor} strokeWidth="10" strokeDasharray="440" strokeDashoffset={offset} strokeLinecap="round" style={{ transition: "stroke-dashoffset 1.5s var(--ease-out)" }} />
        </svg>
      ...
      {stats.repDeviations && stats.repDeviations.length > 0 && (
        <div className="glass animate-in" style={{ width: '100%', maxWidth: '600px', padding: '20px', marginBottom: '20px' }}>
          <div style={{ fontSize: '0.65rem', color: 'var(--neon-yellow)', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '20px', fontWeight: 700, textAlign: 'left' }}>FORM FATIGUE (POSTURE DEVIATION)</div>
            ...
            {stats.repDeviations.map((dev, index) => {
              ...
              return (
                <div key={index} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, height: '100%', justifyContent: 'flex-end', gap: '4px' }}>
                  ...
                  <div style={{ width: '60%', maxWidth: '20px', height: `${heightPct}%`, background: color, borderRadius: '2px 2px 0 0', boxShadow: `0 0 8px ${color}44`, transition: 'height 1s ease-in-out', minHeight: '4px' }}></div>
      ...
      {/* VBT Fatigue Degradation Chart */}
      <VBTDegradationChart velocitiesSession={stats.velocitiesSession} />
      ...
      {stats.jumpingJackSync?.score !== null && stats.jumpingJackSync?.score !== undefined && (
        <div className="glass animate-in" style={{ width: '100%', maxWidth: '600px', padding: '20px', marginBottom: '20px', borderTop: '2px solid var(--neon-cyan)' }}>
          <div style={{ fontSize: '0.65rem', color: 'var(--neon-cyan)', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '16px', fontWeight: 700, textAlign: 'left' }}>JUMPING JACK COORDINATION</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', textAlign: 'center' }}>
            <div><div style={{ color: '#fff', fontSize: '1.6rem', fontWeight: 900 }}>{stats.jumpingJackSync.score}%</div><div style={{ fontSize: '0.55rem', color: 'var(--text-dim)', letterSpacing: '1px', textTransform: 'uppercase' }}>Sync Score</div></div>
            <div><div style={{ color: stats.jumpingJackSync.lagMs && stats.jumpingJackSync.lagMs > 0 ? 'var(--neon-yellow)' : 'var(--neon-green)', fontSize: '1.6rem', fontWeight: 900 }}>{stats.jumpingJackSync.lagMs ? `${Math.abs(stats.jumpingJackSync.lagMs)}ms` : '0ms'}</div><div style={{ fontSize: '0.55rem', color: 'var(--text-dim)', letterSpacing: '1px', textTransform: 'uppercase' }}>{stats.jumpingJackSync.lagMs && stats.jumpingJackSync.lagMs < 0 ? 'Arms Lead' : 'Arm Lag'}</div></div>
            <div><div style={{ color: '#fff', fontSize: '1.6rem', fontWeight: 900 }}>{Math.round(stats.jumpingJackSync.confidence * 100)}%</div><div style={{ fontSize: '0.55rem', color: 'var(--text-dim)', letterSpacing: '1px', textTransform: 'uppercase' }}>Confidence</div></div>
          </div>
        </div>
      )}
      ...
      {stats.gainedXp ? (
        <div className="glass animate-in" style={{ width: '100%', maxWidth: '600px', padding: '20px', marginBottom: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', borderColor: 'var(--neon-yellow)', background: 'rgba(255, 235, 59, 0.05)' }}>
          ...
          {leveling && (<div style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '15px' }}><span style={{ color: 'var(--text-dim)', fontSize: '0.8rem', fontWeight: 'bold' }}>LVL {leveling.level}</span><div style={{ flex: 1, height: '8px', background: 'rgba(255,255,255,0.1)', borderRadius: '4px', overflow: 'hidden' }}><div style={{ width: `${leveling.progress}%`, height: '100%', background: 'var(--neon-yellow)' }}></div></div><span style={{ color: 'var(--text-dim)', fontSize: '0.8rem' }}>{leveling.nextLevelXp} XP</span></div>)}
        </div>
      ) : null}

      {stats.calories !== undefined && stats.calories > 0 && (
        <div className="glass animate-in" style={{ width: '100%', maxWidth: '600px', padding: '20px', marginBottom: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderColor: 'var(--neon-green)', background: 'rgba(0, 255, 100, 0.04)', flexWrap: 'wrap', gap: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><span style={{ fontSize: '2rem' }}>🔥</span><div><div style={{ fontSize: '0.65rem', color: 'var(--neon-green)', letterSpacing: '2px', textTransform: 'uppercase', fontWeight: 700, marginBottom: '4px' }}>Est. Calories Burned</div><div style={{ color: '#fff', fontSize: '2rem', fontWeight: 900, fontFamily: 'var(--font-heading)', lineHeight: 1 }}>{stats.calories}<span style={{ fontSize: '1rem', color: 'var(--text-dim)', marginLeft: '4px' }}>kcal</span></div></div></div>
          <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <div style={{ fontSize: '0.65rem', color: 'var(--text-dim)', letterSpacing: '1px', textTransform: 'uppercase' }}>Accuracy Impact</div>
            <div style={{ fontSize: '0.85rem', color: stats.accuracy > 75 ? 'var(--neon-green)' : 'var(--neon-yellow)', fontWeight: 700 }}>
              {stats.accuracy > 75 ? '✅ Full credit' : stats.accuracy > 50 ? '⚠️ Reduced (form)' : '⬇️ Low (poor form)'}
            </div>
            <div style={{ fontSize: '0.7rem', color: 'var(--text-dim)' }}>MET-based estimate</div>
          </div>
        </div>
      )}
      ...
      <div className="glass animate-in" style={{ width: '100%', maxWidth: '600px', padding: '20px', marginBottom: '20px' }}>
        <div style={{ fontSize: '0.65rem', color: 'var(--neon-cyan)', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '20px', fontWeight: 700, textAlign: 'left' }}>WEEKLY ACTIVITY (AVG ACCURACY)</div>
        {hasWeeklyActivity ? (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', height: '140px', padding: '0 10px', paddingTop: '10px' }}>
            {weeklyData.map((item, index) => (
              <div key={index} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, height: '100%', justifyContent: 'flex-end' }}>
                <span style={{ fontSize: '0.65rem', color: '#fff', marginBottom: '4px', opacity: 0.8 }}>{item.score}%</span>
                <div style={{ height: '80px', width: '100%', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
                  <div style={{ width: '70%', maxWidth: '30px', height: `${item.score}%`, background: index === 6 ? 'linear-gradient(to top, var(--neon-purple), var(--neon-cyan))' : 'var(--neon-cyan)', borderRadius: '4px 4px 0 0', boxShadow: index === 6 ? '0 0 15px var(--neon-purple)' : '0 0 10px var(--neon-cyan)44', transition: 'height 1s ease-in-out', minHeight: '5px' }}></div>
                </div>
                <span style={{ fontSize: '0.65rem', color: 'var(--text-dim)', marginTop: '8px', textTransform: 'uppercase' }}>{item.day}</span>
              </div>
            ))}
          </div>
        ) : (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '140px', color: 'var(--text-dim)', fontSize: '0.85rem', textAlign: 'center', padding: '0 20px' }}>No activity yet. Complete a workout to start your weekly trend.</div>)}
      </div>
      <div className="animate-in glass" style={{ width: '100%', maxWidth: '600px', padding: '15px', textAlign: 'center', marginBottom: '40px', borderColor: accuracyColor }}><div style={{ color: accuracyColor, fontWeight: 700, fontSize: '0.8rem', letterSpacing: '2px' }}>SESSION RATING: {getPerformanceHighlight()}</div></div>
      ...
      {stats.tags && stats.tags.length > 0 && (
        <div className="animate-in glass" style={{ width: "100%", maxWidth: "600px", padding: "20px", marginBottom: "30px", background: "rgba(157, 78, 221, 0.05)", borderStyle: "dashed", borderColor: "var(--neon-purple)" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", justifyContent: "center" }}>{stats.tags.map((tag) => (<span key={tag} style={{ background: "rgba(157, 78, 221, 0.15)", color: "#fff", padding: "6px 14px", borderRadius: "20px", fontSize: "0.75rem", fontWeight: 600, border: "1px solid var(--neon-purple)", boxShadow: "0 0 10px rgba(157, 78, 221, 0.3)" }}>{tag.toUpperCase()}</span>))}</div>
        </div>
      )}

      <div className="animate-in" style={{ width: "100%", maxWidth: "600px", marginBottom: "24px", padding: "20px", borderRadius: "16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", textAlign: "center" }}>
        <h3 style={{ color: "#00e0ff", marginBottom: "12px", fontSize: "1.3rem" }}>🔥 Workout Streak</h3>
        <p style={{ color: "#ffffff", fontSize: "1.1rem", marginBottom: "8px" }}>Current Streak: {streakData.currentStreak} days</p>
        <p style={{ color: "#bbbbbb", fontSize: "0.95rem" }}>Longest Streak: {streakData.longestStreak} days</p>
      </div>

      <div className="animate-in" style={{ display: "flex", gap: "20px", width: "100%", maxWidth: "600px", pointerEvents: "all" }}>
        <button onClick={onRestart} className="btn-outline" style={{ flex: 1 }}><RotateCcw size={16} /> RESTART</button>
        <button onClick={onViewReplay} className="btn-neon" style={{ flex: 1, background: "var(--neon-purple)", color: "#fff" }}>VIEW 3D REPLAY <Video size={16} /></button>
      </div>
    </div>
  );
};